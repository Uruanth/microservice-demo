...
import org.example.domain.events.AccountNameEdited;
...
public class MaterializeLookUp {
    ...
    public MaterializeLookUp(ApplicationContext context, AccountRepository repository) {
        ...
        business.put("org.example.AccountNameEdited", Flux.just(input -> {
            var event = (AccountNameEdited) input;
            return repository.findById(event.aggregateRootId())
                    .flatMap(doc -> {
                        var document = new AccountModeView();
                        document.setId(event.aggregateRootId());
                        document.setName(event.getName().value());
                        document.setUserId(doc.getUserId());
                        document.setTransactionModelViews(doc.getTransactionModelViews());
                        return repository.save(document).then();
                    });
        }));
