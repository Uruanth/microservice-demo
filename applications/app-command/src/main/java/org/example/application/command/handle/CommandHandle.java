...
import org.example.business.EditAccountNameUseCase;
...
public class CommandHandle {
    ...
    @Bean
    public RouterFunction<ServerResponse> editName(EditAccountNameUseCase usecase) {

        return route(
                POST("/account/edit").and(accept(MediaType.APPLICATION_JSON)),

                request -> usecase.andThen(integrationHandle)
                        .apply(request.bodyToMono(EditAccountNameCommand.class))
                        .then(ServerResponse.ok().build())
                        .onErrorResume(errorHandler::badRequest)
        );
    }
